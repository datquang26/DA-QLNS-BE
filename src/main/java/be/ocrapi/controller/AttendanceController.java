...
//    }
    ...
    public BaseResponse<?> findAll(
        ...
        try {
            ...
            int offset = (pageInt - 1) * pageSizeInt;  // Tính offset chính xác

            var response = service.findAndCount(offset, pageSizeInt, from_date, to_date, status, user_id, type);

            BaseResponse.Metadata paging = new BaseResponse.Metadata("", pageInt - 1 , pageSizeInt, response.getTotal(), "", null);
